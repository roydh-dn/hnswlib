...
void get_gt(unsigned int *massQA, float *massQ, float *mass, size_t vecsize, size_t qsize, L2Space &l2space,
    ...
    for (int i = 0; i < qsize; i++) {
        for (int j = 0; j < k; j++) {
            float other =
                fstdistfunc_(massQ + i * vecdim, mass + massQA[100 * i + j] * vecdim, l2space.get_dist_func_param());
...
#pragma omp parallel for
    ...
    //        }
    //        appr_alg.setEf(400);
    //        std::priority_queue< std::pair< float, tableint >> closest_elements =
    //        appr_alg.searchKnnInternal(appr_alg.getDataByInternalId(i), 17); while (closest_elements.size() > 0) {
    ...
    //
    //        for (tableint l : g) {
    //            float other = fstdistfunc_(appr_alg.getDataByInternalId(l), appr_alg.getDataByInternalId(i),
    //            l2space.get_dist_func_param()); rez.emplace(other, l);
    //        }
    //        while (rez.size() > 32)
    //            rez.pop();
    //        int len = rez.size();
