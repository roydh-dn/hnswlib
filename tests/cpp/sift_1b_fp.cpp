...
static void get_gt(unsigned int *massQA, size_t vecsize, size_t qsize, size_t vecdim,
                   vector<std::priority_queue<std::pair<float, labeltype>>> &answers, size_t k) {
    (vector<std::priority_queue<std::pair<float, labeltype>>>(qsize)).swap(answers);
    cout << "qsize: " << qsize << "\n";
    for (int i = 0; i < qsize; i++) {
        for (int j = 0; j < k; j++) {
            answers[i].emplace(0.0f, static_cast<float>(massQA[1000 * i + j]));
        }
    }
}
...
void sift_test1B_fp() {
    ...
    snprintf(path_index, sizeof(path_index), "sift1b_%dm_ef_%d_M_%d_fp.bin", subset_size_milllions, efConstruction, M);
    ...
    for (int i = 0; i < qsize; i++) {
        ...
        for (int j = 0; j < vecdim; j++) {
            massQ[i * vecdim + j] = static_cast<float>(massb[j]);
        }
    }
    inputQ.close();
    ...
    if (exists_test(path_index)) {
        ...
        for (int j = 0; j < vecdim; j++) {
            mass[j] = static_cast<float>(massb[j] * (1.0f));
        }
...
#pragma omp critical
            {
                ...
                input.read((char *)massb, in);
                for (int j = 0; j < vecdim; j++) {
                    mass[j] = static_cast<float>(massb[j]);
                }
                j1++;
                j2 = j1;
    ...
    size_t k = 1;
    cout << "Parsing gt:\n";
    get_gt(massQA, vecsize, qsize, vecdim, answers, k);
    cout << "Loaded gt\n";
    for (int i = 0; i < 1; i++)
